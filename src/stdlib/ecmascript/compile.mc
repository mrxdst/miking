...
lang MExprESCompile = MExprAst + ESAst + MExprPrettyPrint + MExprArity
  ...
  | DeclLet d ->
    ...
    if null (nameGetStr d.ident) then
      ...
      -- An alias of a function inherits its arity, so calls through it stay
  ...
  | const ->
    ...
    -- with `--native-parser`. Code that uses the boot parser is still reached,
    -- `mexpr/boot-parser.mc` is included by the compiler itself, so these must
    -- compile to something; a stub keeps that code legal and makes an actual
    -- call fail immediately, naming the intrinsic.
